...
movies <- transmute(movies,
                    ...
                    Format = as.factor("Film"),
                    Status = as.factor("Ended"))
...
natural_joined <- mutate(natural_joined, `Format.x` = levels(formats)[ifelse(is.na(`Format.x`), `Format.y`, `Format.x`)])
natural_joined <- mutate(natural_joined, `Status.x` = levels(statuses)[ifelse(is.na(`Status.x`), `Status.y`, `Status.x`)])
natural_joined <- mutate(natural_joined, Country = ifelse(is.na(Country), Region, Country))

natural_joined <- natural_joined[, !(names(natural_joined) %in% c("Genre.y", "Language.y", "Region", "Format.y", "Status.y"))]
...
unique(select(natural_joined, Genre))
```
