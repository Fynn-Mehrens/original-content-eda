...
To include a regression model that is accurate in its values, we originally wanted to create a model that analyses the invested resources put into the original content and observe the outcome and compare that model with one that only observes content produced by any streaming service. Sadly, there is not enough precise data that we could find or access, but we wanted to include a more precise one either way.

Since IMDb and Rotten Tomatoes rate movies and series individually, we wanted to observe whether we can find rather strong or weak correlations. We multiplied the IMDb score by ten to map it to the Rotten Tomatoes percentage for this model.
...
For the plots, we can observe that the model does not have very strong outliers for the middle to high values, and the plots do not show any obvious patterns that would suggest that a curvilinear approach is more fitting to model the relationship between the two rating services.
