...
natural_joined <- mutate(natural_joined, `Format.x` = formats[[`Format.x`[1]]])
natural_joined <- mutate(natural_joined, `Status.x` = ifelse(is.na(`Status.x`), `Status.y`, `Status.x`))
natural_joined <- mutate(natural_joined, `Status.x` = statuses[[`Status.x`[1]]])
natural_joined <- mutate(natural_joined, Country = ifelse(is.na(Country), Region, Country))
natural_joined <- mutate(natural_joined, Min_Time = ifelse(is.na(Min_Time), Runtime, Min_Time))
natural_joined <- mutate(natural_joined, Max_Time = ifelse(is.na(Max_Time), Runtime, Max_Time))
...
```{r}
natural_joined2 <- full_join(shows, natural_joined, by = "Title")

natural_joined2 <- mutate(natural_joined2, `Year.x` = ifelse(is.na(`Year.x`), `Year.y`, `Year.x`))
natural_joined2 <- mutate(natural_joined2, `Age.x` = ifelse(is.na(`Age.x`), `Age.y`, `Age.x`))
natural_joined2 <- mutate(natural_joined2, `IMDb.x` = ifelse(is.na(`IMDb.x`), `IMDb.y`, `IMDb.x`))
natural_joined2 <- mutate(natural_joined2, `Rotten Tomatoes.x` = ifelse(is.na(`Rotten Tomatoes.x`), `Rotten Tomatoes.y`, `Rotten Tomatoes.x`))
natural_joined2 <- mutate(natural_joined2, `Netflix.x` = ifelse(is.na(`Netflix.x`), `Netflix.y`, `Netflix.x`))
natural_joined2 <- mutate(natural_joined2, `Hulu.x` = ifelse(is.na(`Hulu.x`), `Hulu.y`, `Hulu.x`))
natural_joined2 <- mutate(natural_joined2, `Prime Video.x` = ifelse(is.na(`Prime Video.x`), `Prime Video.y`, `Prime Video.x`))
natural_joined2 <- mutate(natural_joined2, `Disney+.x` = ifelse(is.na(`Disney+.x`), `Disney+.y`, `Disney+.x`))
natural_joined2 <- mutate(natural_joined2, `Apple TV+.x` = ifelse(is.na(`Apple TV+.x`), `Apple TV+.y`, `Apple TV+.x`))
natural_joined2 <- mutate(natural_joined2, `Format.x` = as.numeric(ifelse(is.na(`Format.x`), `Format.y`, `Format.x`)))
natural_joined2 <- mutate(natural_joined2, `Format.x` = formats[[`Format.x`[1]]])
natural_joined2 <- mutate(natural_joined2, `Status.x` = as.numeric(ifelse(is.na(`Status.x`), `Status.y`, `Status.x`)))
natural_joined2 <- mutate(natural_joined2, `Status.x` = statuses[[`Status.x`[1]]])

natural_joined2 <- natural_joined2[, !(names(natural_joined2) %in% c("Year.y", "Age.y", "IMDb.y", "Rotten Tomatoes.y", "Netflix.y", "Hulu.y", "Prime Video.y", "Disney+.y", "Apple TV+.y", "Format.y", "Status.y"))]

(natural_joined2 <- rename(natural_joined2, Year = `Year.x`, Age = `Age.x`, IMDb = `IMDb.x`, `Rotten Tomatoes` = `Rotten Tomatoes.x`, Netflix = `Netflix.x`, Hulu = `Hulu.x`, `Prime Video` = `Prime Video.x`, `Disney+` = `Disney+.x`, `Apple TV+` = `Apple TV+.x`, Format = `Format.x`, Status = `Status.x`))
```
