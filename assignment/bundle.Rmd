...
To exclude specific rows that do not match one of the specified statuses formats or origins, the only_keep() function was written to ease parsing the factors after matching parts of the previous entry to unify the content of the column.
...
The show data set comes, in comparison to the movies data set, with a lot less columns, only eleven to be precise. We are not making use of the same three columns that we did not make use of with the movies data set.
...
For the transformation process, we were in need of writing functions that are capable of handling the input we are given in the exact manner we want it to be handled, since we could not find any base R function or any package that would be capable of doing the same thing.
...
The **filter_originals()** function has been introduced with the thought in mind that there are certain rows that contain ambiguous data that we would not want to keep, which is why we are matching against the RegExs that can be found in the scope of the **filter()** within the body of the function.
...
When converting the data into the correct data type and columns that are easy to deal with, we started off by testing our **transmute()** function usage against the results of the **netflix_originals** data frame and adapted for each time where we found an unwanted result with any of the other streaming services. This lead to the **transmute_originals()** function that can now be seen.
...
The filtering and transmutation process is now applied to all of the individual original content data frames, which leads to the results that are shown below.
...
To ensure that the data frames that hold the original content can not be confused with the data frames that originally hold the non-transmuted content, they are removed from the pool of global variables.
...
The goal now is to combine the results from the transmutation into one large data frame that can then be explored to gain a general grasp of what the data looks like for originals from five of the largest streaming services. The result is a data frame with 3979 rows.
...
Joining the now larger data frame with the imported shows puts up some more issues since the previously merged movies data frame contained a lot of columns that are also known to the shows data frame, but can never hold intersecting data, since an entry can never be a film and a series at the same time. Therefore the following columns are merged accordingly:
...
Because Apple TV+ only holds original content, there is no need to worry about us including it, although it is not mentioned in the movies or shows data set. One important piece of information we are lacking with this approach is the age restriction of the productions. Therefore we scraped the age restrictions from the [IMDb page](https://www.imdb.com/search/title/?companies=co0546168) and combined them with the data we have.
...
After looking at the outcome of the merging process, we also realized that there are no entries for the availability on other platforms for the Apple TV+ productions, so we made sure to add them as well.
...
For the data exploration portion of the analysis, we are using custom color schemes for the FSK colors and colors from the logos to represent the streaming services in the plots accordingly.
...
To ease the creation of some plots that we just alter in very small fashion, we created some functions that create these plots for us and take the altering variables as parameters.
...
<br/>
What we can see for the development of releases throughout the years is that besides Netflix's production in 2005 the original content started its rise in 2010 and the production rate increased exponentially for the first 7-8 years. From there on the content production still experienced a linear increase.

## Linear regression model for the amount of original content until 2050

We were also interested in finding out whether we could predict the number of productions based on the year even though the year logically can not be the cause of the number of productions. We are also aware that the amount of observations is way to low to create a good regression model, but we wanted to include this nevertheless.
...
For Netflix we can see that the ratio between distribution and original content has risen to almost 75% for the year of 2020 which tells us that Netflix tries to avoid the licensing costs of content that they did not produce themselves.
...
Hulu, just like Amazon Prime is very late to start with mass production of original content as it is sitting at around 25% for the year 2020.
...
In contrast to the three streaming services shown beforehand Disney entered the market for content streaming very recently, in the October of 2020. They were very aware that the production of original content is one of the major changes in the series and movies industry as of now, which is why they released with over 25% original content  which they brought up to a little over 60% in 2020 already.
...
With Apple TV+ we can see a company that fully embraces the production of original content. From their start in 2019 they have not released any single production that was not produced by them.
...
```
<br/>
We can see that there is indeed a spike for the releases during the autumn period with larger drops right afterwards as well as in the middle of summer. The November spike is artificially increased by the Disney+ releases since they released with a lot of content in November of 2019.
...
When looking at the distribution of age ratings it is very well recognizable that there is a lot of content marked with a 0+ age restriction which is probably done to be able to address as many people as possible. You can also see on overall increase in the 18+ category, which seems to be the second largest overall. It is also interesting to note that there are no series with an age rating of 12+.
...
Netflix does not seem to follow any particular trend on their own, they rather follow the norm.
...
Besides an unusually large amount of 0+ productions, Amazon Prime Video also seems to follow the norm with variations in every year, but that can obviously be expected. What we can derive from this is that the age restrictions on the original content are rather bound to the image the company or streaming service had beforehand or wants to strive towards.
...
For Hulu, we can see an amount of 18+ rated productions that is very high in relation to how large the percentile of 18+ productions of the other streaming services is. Especially for 2019, the percentage of 18+ productions is sitting at almost 50 percent.
...
relevant_years <- c(2017:2020)
revenue <- read.csv("../datasets/yearly_revenue.csv")
users <- read.csv("../datasets/yearly_users.csv")

regression_df <- data_frame()

for (year in relevant_years) {
  regression_df %<>% rbind(data_frame(Year = year, Productions = nrow(filter(content, Year == year))))
}

(regression_df %<>% mutate(Revenue = revenue$Revenue, Users = users$Users))

regression_model <- lm(regression_df$Productions ~ regression_df$Users + regression_df$Revenue, data = regression_df)

summary(regression_model)

cor(regression_df$Productions, regression_df$Users)
cor(regression_df$Productions, regression_df$Revenue)

cor(regression_df$Users, regression_df$Revenue)
...
```
```{r}
age_regression <- lm(Seasons ~ IMDb, data = filter(content, Format == "Series", Service == "Netflix"))
