...
The filtering and transmutation process is now applied to all original content data frames, leading to the results shown below.
...
After looking at the merging process's outcome, we also realized that there are no entries for the availability on other platforms for the Apple TV+ productions, so we made sure to add them.
...
For the data exploration portion of the analysis, we use custom color schemes for the FSK colors and colors from the logos to represent the plots' streaming services accordingly.
...
## Linear regression model for the amount of original content
...
Looking at the summary of this linear regression, we can see that the model explains about 66 percent of the varying values, but the degrees of freedom are very low, at 14. The standard error on this model is also way too high to make any predictions, even if the x variable was responsible for changing production numbers.
...
To include a regression model, that is accurate in its values we originally wanted to create a model that analyses the invested resources put into the original content and observe the outcome and compare that model with one that only observes content that is not produced by any streaming service. Sadly, there is not enough precise data that we could find or access, but we wanted to include a more precise one either way.

Since IMDb and Rotten Tomatoes rate movies and series individually, we wanted to observe whether we can find rather strong or weak correlations. For this model we multiplied the IMDb score by ten to map it to the Rotten Tomatoes percentage.

While we can see that the r-squared value is relatively low in comparison to the previous regression model, it is still high enough for our purpose. The residual standard error only lies at around 8.5 percent points which is pretty accurate for what we are observing.
...
For the plots, we can observe that for the middle to high values the model does not have very strong outliers and the models do not show any obvious patterns that would suggest that a curvilinear approach is more fitting to model the relationship between the two rating services.
