...
#content <- mutate(content, `Format.x` = formats[[`Format.x`[1]]])
content <- mutate(content, `Status.x` = as.integer(ifelse(is.na(`Status.x`), `Status.y`, `Status.x`)))
#content <- mutate(content, `Status.x` = statuses[[`Status.x`[1]]])
...
content <- mutate(content, Month = ifelse(is.na(Premiere), NA, as.numeric(format(Premiere, "%m"))))
content <- mutate(content, Day = ifelse(is.na(Premiere), NA, as.numeric(format(Premiere, "%d"))))

content <- mutate(content, Age = parse_factor(ifelse(is.na(Age), "unknow", Age), levels = ages))
content <- mutate(content, Origin = parse_factor(ifelse(is.na(Origin), "Distribution", Origin), levels = origins))

#parse_factor(only_keep(Origin, "(Continuation|Distribution|Original)"), levels = origins)

unique(select(content, Format))
...
unique(select(content, Format))
unique(select(content, Age))
unique(select(content, Origin))
unique(select(content, Service))
```
